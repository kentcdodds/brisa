...
import buildClientPage from './3-client-pages/build-page';
...
import { transpileActions } from '@/build-process/2-entrypoints/2-server-actions/transpilation';
import { buildActions } from '@/build-process/2-entrypoints/2-server-actions/build-actions';
...
export default async function compileEntrypoints() {
  const {
    SRC_DIR,
    BUILD_DIR,
    CONFIG,
...
async function compileClientCodePage(
  ...
  for (const page of pages) {
    ...
    const pageCode = await buildClientPage({
      pagePath,
      allWebComponents,
      pageWebComponents,
      integrationsPath,
