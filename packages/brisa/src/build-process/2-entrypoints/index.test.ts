...
import compileEntrypoints from '.';
...
describe('build process -> 2. entrypoints', () => {
  beforeEach(() => {
  ...
  describe('compileEntrypoints DEVELOPMENT', () => {
    afterEach(() => {
      globalThis.mockConstants = undefined;
    });
    it('should compile fixtures routes correctly', async () => {
      ...
      const { success, logs } = await compileEntrypoints();
      const files = fs
  ...
  describe('compileEntrypoints PRODUCTION', () => {
    it('should compile fixtures routes correctly', async () => {
  ...
  `);
    ...
    ${info}Route                        | JS server | JS client (gz)  
    ${info}-------------------------------------------------------
    ${info}λ /pages/index               | 444 B   | ${greenLog('0 B')}
    ${info}λ /pages/pokemon/[slug]      | 512 B   | ${greenLog('0 B')}
  ...
  `);
    ...
    ${info}λ /pages/page-without-web-component  | 444 B      | ${greenLog('0 B')}  
    ${info}
    ${info}λ Server entry-points
    ${info}Φ JS shared by all
    ${info}
